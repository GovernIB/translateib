package es.caib.translatorib.commons.i18n;
 ...
 * paràmetres. Els paràmetres a la seva vegada poden ser etiquetes si estan
 * entre claus, p.e. <code>{camp.nom}</code>. Si no s'indica el
 * <code>locale</code> per la trauducció agafarà {@link Locale#getDefault()}.
 * <p>
 * Les intàncies s'obtenen cridant {@link #from(String...)}, passant per
 * paràmetre noms de recursos que es carregaran com a {@link ResourceBundle} per
 * emprar com a recursos per fer la traducció.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * Fixau-vos que el pàmetre <code>"error.tamany"</code> no està envolutat de
 * <code>{}</code> perquè aquest paràmetre sempre és una etiqueta que es
...
public class I18NTranslator implements Serializable {
	...
	/**
	 * Instància de ResourceBundle.Control emprada per carrega els bundles. Atès que
	 * la instància és reutilitzable la declaram aquí perque sigui única.
	 */
	private static final ResourceBundle.Control NO_FALLBACK_CONTROL = new NofallbackControl();

	/**
	 * Cache d'instàncies de I18NTranslator. A cada llista de String que emprem li
	 * correspondrà una única instància.
	 */
	private static final ConcurrentMap<List<String>, I18NTranslator> INSTANCES = new ConcurrentHashMap<List<String>, I18NTranslator>();

	/**
	 * Caché de ResourceBundle de cada instància de I18NTranslator
	 */
	private final ConcurrentMap<Locale, ResourceBundle> bundleMap = new ConcurrentHashMap<Locale, ResourceBundle>();
	 ...
	 */
	public static I18NTranslator from(final String... bundleNames) {
...
//		}); 
	 ...
	 *         l'etiqueta.
	 */
	private String processStringParam(final Locale locale, final String string) {
		if (string.startsWith("{") && !string.contains(" ") && !string.contains("=") && string.endsWith("}")) {
	 ...
	 *         existeix traducció.
	 */
	public String translate(final Locale locale, final String label, final Object... parameters) {
		String message;
		try {
...
//						.toArray();
	 ...
	 */
	public String translate(final Locale locale, final String label) {
	 ...
	 * @see #translate(Locale, String, Object...)
	 */
	public String translate(final String label, final Object... parameters) {
	 ...
	 * @see #translate(Locale, String, Object...)
	 */
	public String translate(final String label) {
		return translate(Locale.getDefault(), label, (Object[]) null);
	}

	/**
	 * Subclasse de Control interna emprada per carregar els ResourceBundle per que
	 * en cas que no trobi el locale indicat no empri el locale per defecte.
	 */
