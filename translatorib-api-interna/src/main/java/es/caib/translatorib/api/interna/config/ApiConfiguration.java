...
/**
 * Configuració de l'aplicació JAX-RS. Es pot fer mitjançant aquesta subclasse
 * de {@link Application} o mitjançant la configuració a web.xml. Fer-ho amb una
 * subclasse permet posar-hi anotacions de OpenApi. Aquí fixam la informació
 * general d'OpenAPI, la ubicació del servidor (o servidors) que bàsicament és
 * el contextpath, els sistemes d'autenticació admesos....
 * <p>
 * Al package services hi ha les classes amb els endpoints dels recursos.
 * Segueix les convencions més comunes de les apis REST, es pot trobar una bona
 * descripció aquí: https://restfulapi.net/http-methods/ Els mètodes de consulta
 * GET, retornen un 200 amb la informació o 404 si no es troba. Els mètodes de
 * creació, POST, retornen un 201 amb la capçalera 'location' cap al nou recurs.
...
@OpenAPIDefinition(info = @Info(title = "API REST", version = "2.0.0"), servers = {
		@Server(url = "/translatoribapi/interna") }, security = { @SecurityRequirement(name = "basicAuth"),
